...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function (exports) {
    ...
    var $ = layui.$
        ...
        , table = {
            ...
            , updateRow: function (tableId, obj) {
                ...
                /**
                 * TODO
                 * 更新行信息的时候 行号Number会根据 数组 下标重新赋值 所以需要加1 ， 至于影响后期观察
                 * 目前只是修改 updateRow时 更新 的自定义
                 *
                 */
                var numbers =  oi+1 ;

                var tds = that.renderTr(thisobj, numbers);
            ...
            , exportFile: function (tableId, data, t) {
                var that = table.getClass(tableId)
                that.exportFile(tableId, data, t)
            }
        }
        //操作当前实例
        , thisTable = function () {
        ...
        , TPL_HEADER = function (options) {
            ...
            return ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" '
                ...
                /**
                 * TODO  <th data-field ..> 增加下面的data-key 用于隐藏列操作
                 * data-key="{{d.index}}-{{i1}}-{{i2}}" {{# if( item2.parentKey){ }}data-parentkey="{{ item2.parentKey }}"{{# } }}
                 */
                , '<th data-field="{{ item2.field||i2 }}" data-key="{{d.index}}-{{i1}}-{{i2}}" {{# if( item2.parentKey){ }}data-parentkey="{{ item2.parentKey }}"{{# } }} {{# if(item2.minWidth){ }}data-minwidth="{{item2.minWidth}}"{{# } }} ' + rowCols + ' {{# if(item2.unresize){ }}data-unresize="true"{{# } }}>'
    ...
    Class.prototype.renderToolbar = function () {
        ...
        var that = this, options = that.config,
            ...
            n = that.layTool.find(".layui-table-tool-temp");
        console.info(l);


        if ("default" === options.toolbar) {
            n.html(l);
        }else if ("string" == typeof options.toolbar) {
            //laytpl($(options.toolbar).html() || '').render(options);
            var o = $(options.toolbar).html() || "";
            console.info(o);
            o && n.html(laytpl(o).render(options))
        }
        ...
        "object" == typeof options.defaultToolbar && layui.each(options.defaultToolbar, function (e, t) {
            var i = r[t];
            i && d.push('<div class="layui-inline" title="' + i.title + '" lay-event="' + i.layEvent + '"><i class="layui-icon ' + i.icon + '"></i></div>')
        }), that.layTool.find(".layui-table-tool-self").html(d.join(""))
    };

    /**
     * TODO 用于隐藏列 【貌似在 tree中没有发现 作用】
     * @param e
     * @param t
     */
    Class.prototype.setParentCol = function (e, t) {
        ...
        if (l[0]) {
            ...
            e ? n-- : n++, l.attr("colspan", n), l[n < 1 ? "addClass" : "removeClass"](HIDE), r.colspan = n, r.hide = n < 1;
    ...
    /**
     * TODO 导出Excel 和 Cvs文件
     * @param e
     * @param t
     * @param i
     * @returns {*}
     */
    Class.prototype.exportFile = function (e, t, i) {
        var that=this;

        t = t || table.clearCacheKey(table.cache[e]), i = i || "csv";
        var a = that.config[e] || {}, l = {csv: "text/csv", xls: "application/vnd.ms-excel"}[i],
            n = document.createElement("a");
        return device.ie ? hint.error("IE_NOT_SUPPORT_EXPORTS") : (n.href = "data:" + l + ";charset=utf-8,\ufeff" + encodeURIComponent(function () {
            var i = [], a = [];
            console.info(t);
            return layui.each(t.data.list, function (t, l) {
                var n = [];
                "object" == typeof e ? (layui.each(e, function (e, a) {
                    0 == t && i.push(a || "")
                }), layui.each(table.clearCacheKey(l), function (e, t) {
                    n.push(t)
                })) : that.eachCols(function (e, a) {
    ...
    Class.prototype.renderTdEdit = function (index, field) {
        var that = this
            , othis = $(that.getTd(index, field))
            , options = that.config
            , field = othis.data('field')
            , editType = othis.data('edit')
            , index = othis.parents('tr').eq(0).data('index')
            , val = othis.find("div.layui-table-cell p").text()
    ...
    Class.prototype.renderTd = function (obj, cols, numbers, i3) {
        ...
        var td = ['<td data-field="' + field + '" ' + function () {
            var attr = [];
            attr.push('data-key="' + options.index + '-' + cols.key + '"');
    ...
    Class.prototype.tableHeight = function () {
        ...
        if (!table.kit.isNumber(optionsFirst.height)) {//如果非固定高度，则计算高度
            ...
            if (options.height) {
                if (/^full-\d+$/.test(optionsFirst.height)) {
                    that.fullHeightGap = optionsFirst.height.split('-')[1];
                }
            }
    ...
    Class.prototype.events = function () {
        ...
        /**
         * TODO 监听数据标记 页面document 时 隐藏 defaultToobar 选择操作展示框
         */
    ...
    Class.prototype.eventsinit = function () {
            ...
            if (isAll) {
                ...
                if (options.isTree) {
                    ...
                    /*var temf = function (o) {
                        ...
                        if (o && o.children.length > 0) {
                            ...
                            o.children.forEach(function (temo) {
                                console.info('-----temo-----');
                                console.info(temo);
                                console.info('----table.config.cols.isCheckName---');
                                console.info(table.config.cols.isCheckName);
                    ...
                    var temf = function (o) {
                        ...
                        /**
                         * TODO 自定义修改
                         * 当前触发事件节点如果是选中 就依次选中 父节点
                         * 如果触发事件节点 取消选中 才判断是否取消选中父节点 ，当前节点兄弟节点全未选中才取消选中父节点
                         */
                        if(!checked){
                            o.children.forEach(function (temo) {
                                if (temo[table.config.cols.isCheckName]) {
        ...
        that.layBody.on('click', '*[lay-event]', function () {
            ...
            layui.event.call(this, MOD_NAME, 'tool(' + filter + ')', {
                ...
                , tr: tr
                , del: function () {
                    console.info("我是data " + options.id)
                    console.info(data)
        ...
        that.layTool.on('click', '*[lay-event]',function (e) {
            var e,$this = $(this), cevent = $this.attr('lay-event'),
                s = function (e) {
                    var l = $(e.list), n = $('<ul class="layui-table-tool-panel"></ul>');
                    n.html(l), options.height && n.css("max-height", options.height - (that.layTool.outerHeight() || 50)), $this.find(".layui-table-tool-panel")[0] || $this.append(n), that.renderForm(), n.on("click", function (e) {
                        layui.stope(e)
                    }), e.done && e.done(n, l)
                };

            switch (layui.stope(e), $this.trigger("table.tool.panel.remove"), layui.layer.close(that.tipsIndex), cevent) {
                case"LAYTABLE_COLS":
                    s({
                        list: function () {
                            var e = [];
                            ...
                            form.on("checkbox(LAY_TABLE_TOOL_COLS)", function (e) {
                                var i = $(e.elem), l = this.checked, n = i.data("key"), r = i.data("parentkey");
                                layui.each(options.cols, function (e, t) {
                                    layui.each(t, function (t, i) {
                                        if (e + "-" + t === n) {
                                            var d = i.hide;
                                            i.hide = !l, that.elem.find('*[data-key="' + options.index + "-" + n + '"]')[l ? "removeClass" : "addClass"](HIDE), d != i.hide && that.setParentCol(!l, r), that.resize()
                                        }
                                    })
                                })
                            })
                ...
                case"LAYTABLE_EXPORT":
                    device.ie ? layer.tips("导出功能不支持 IE，请用 Chrome 等高级浏览器导出", this, {tips: 3}) : s({
                        list: function () {
                            return ['<li data-type="csv">导出到 Csv 文件</li>', '<li data-type="xls">导出到 Excel 文件</li>'].join("")
                        }(), done: function (e, i) {
                            i.on("click", function () {
                                var e = $(this).data("type");
                                that.exportFile(options.id, null, e)
                            })
                        }
                    });
                    break;
                case"LAYTABLE_PRINT":
                    var h = window.open("打印窗口", "_blank"),
                        ...
                        v = $(that.layHeader.html());
                    v.append(that.layMain.find("table").html()), v.find("th.layui-table-patch").remove(), v.find(".layui-table-col-special").remove(), h.document.write(f + v.prop("outerHTML")), h.document.close(), h.print(), h.close()
            }
